    ...
    if request.method == "POST":
        ...
        if conflict:
            ...
            return render_template(
                "appointment_form.html", clinics=clinics, doctors=doctors
            )

        appt = Appointment(
            doctor_id=doctor_id, patient_id=patient.id, start_at=start_at, end_at=end_at
        )
        db.session.add(appt)
        db.session.commit()
...
def admin_seed():
    ...
    if not Clinic.query.first():
        ...
        db.session.commit()
        created = True
    flash("تم تهيئة البيانات الأولية" if created else "البيانات مُهيّأة مسبقًا")
    return redirect(url_for("admin_dashboard"))
...
# ==================
# التذكيرات (اختياري) — بدون ديكوريتر Flask 3
# ==================
twilio_client = None
scheduler = None
...
def start_scheduler_if_configured():
    """تشغيل المجدول مرة واحدة فقط إذا الإعدادات مكتملة."""
    global twilio_client, scheduler, _scheduler_started
    if _scheduler_started:
        return
    if (
        getattr(app.config, "REMINDERS_ENABLED", False)
        and Client
        and BackgroundScheduler
        and app.config.get("TWILIO_ACCOUNT_SID")
        and app.config.get("TWILIO_AUTH_TOKEN")
        and app.config.get("TWILIO_FROM")
    ):
        try:
            ...
            # نتجاهل أي خطأ هنا حتى لا يوقف التطبيق الأساسي
            print("Scheduler/Twilio init error:", e)
...
def send_reminders():
    ...
    for a in upcoming:
        ...
        try:
            ...
            twilio_client.messages.create(from_=from_num, to=to_num, body=msg)
        except Exception as e:
            print("Twilio error:", e)


# ==================
# نقطة تشغيل التطبيق (إنشاء الجداول دائمًا)
# ==================
@app.cli.command("init-db")
def init_db():
    """أمر يدوي لإنشاء الجداول وإنشاء الأدمن الافتراضي."""
...
if __name__ == "__main__":
    # إنشاء الجداول دائمًا عند التشغيل (يحل مشكلة no such table على Render)
    with app.app_context():
        db.create_all()
        if not Admin.query.filter_by(username="admin").first():
            u = Admin(username="admin")
    ...
    # تشغيل المجدول (اختياري) إذا كانت الإعدادات كاملة
