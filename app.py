...
from sqlalchemy import inspect  # <-- مهم لاكتشاف الجداول
...
def index():
    ...
    ).order_by(Appointment.start_at).limit(10).all()
...
def book():
    ...
    if request.method == 'POST':
        ...
        appt = Appointment(doctor_id=doctor_id, patient_id=patient.id,
...
def send_reminders():
    ...
    for a in upcoming:
        ...
        try:
            # لواتساب: from_=f"whatsapp:{from_num}", to=f"whatsapp:{to_num}"
...
def init_db_if_needed():
    """ينشئ الجداول ويضيف بيانات أولية وأدمن افتراضي إذا لزم الأمر."""
    inspector = inspect(db.engine)
    tables = set(inspector.get_table_names())

    needed = {'admin', 'clinic', 'doctor', 'patient', 'appointment'}
    ...
    if not Clinic.query.first():
        ...
        db.session.flush()
        db.session.add_all([
            Doctor(name='د. أحمد', clinic_id=c1.id),
            Doctor(name='د. سارة', clinic_id=c2.id),
        ])
...
def setup_scheduler():
    # مبدئيًا فعّل المجدول والتذكيرات إن كانت مفاتيح Twilio موجودة
    global client, scheduler
    if app.config.get('REMINDERS_ENABLED') and \
       app.config.get('TWILIO_ACCOUNT_SID') and \
       app.config.get('TWILIO_AUTH_TOKEN') and \
       app.config.get('TWILIO_FROM'):
...
# ==================
# أوامر سريعة للتهيئة الأولى (تبقى متاحة محليًا)
# ==================
...
if __name__ == '__main__':
    with app.app_context():
        init_db_if_needed()  # ← أهم سطر: ينشّط القاعدة عند أول تشغيل
